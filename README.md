...
`catalog.json` 的合集目录）在 hoardodile 里播放出来——按帧播放、动作切换、
按帧触发的角色音频。
...
当前帧/毫秒/绘制层数）、右侧角色音频（按帧列出音效事件，非 1.0 音量显示 `x0.50`
角标、可播放已解析样本、完整事件映射与解析状态）与精灵信息。合集模式下左侧会有
角色选择器。
...
- `src/kernel/**`：**纯函数**（严格函数式）——`layersAt` 采样某时刻的图层、
  `quadFor`/`atlasSource` 图集数学、`dueEvents` 按帧调度音频（含循环回绕）、
  `classifyClips` 按导出端写入的 `group` 分组。
...
- `src/ui/**`：React + `@hoardodile/ui`（只走子路径导入、只用语义 token、
  canvas 绘制走内核返回的几何），渲染循环用 rAF，不套 Effect。canvas 按 DPR 建后备
  缓冲、把四边形对齐**整设备像素**、缩放吸附到整数设备倍数。
